...
public class Uploader
{
  ...
  */
    public Hashtable upFile(HttpContext cxt, string pathbase, string[] filetype, int size)
    {
        ...
        //if (AppPath.GetCurrentVirtualPath() == "")
        //    pathbase = DateTime.Now.ToString("yyyyMMdd") + "/";
        //else
        //    pathbase = AppPath.GetCurrentVirtualPath() + "/" + DateTime.Now.ToString("yyyyMMdd") + "/";

        //uploadpath = MediaServerConfig.Instance.RootPath + pathbase;


        MediaServerPath msPath = new MediaServerPath();
        msPath.RelativePath = DateTime.Now.ToString("yyyyMMdd");

        uploadpath = msPath.FullPath;
        ...
        try
        {
            ...
            Directory.CreateDirectory(msPath.FullPath);
            ...
            if (state == "SUCCESS")
            {
                ...
                msPath.RelativePath += "/" + filename;

                uploadFile.SaveAs(msPath.FullPath);

                //URL = pathbase + filename;

                URL = msPath.RelativeUrl;
            }
        }
        catch (Exception e)
        {
