    ...
    public class JiaYouWxPayApi
    {
        ...
        */
        public static WxPayData OrderQuery(WxPayData inputObj, int timeOut = 6)
        {
            ...
            inputObj.SetValue("appid", WxPayConfig.GetConfig().GetAppID());//公众账号ID
            inputObj.SetValue("mch_id", WxPayConfig.GetConfig().GetMchID());//商户号
            inputObj.SetValue("nonce_str", JiaYouWxPayApi.GenerateNonceStr());//随机字符串
            ...
            DebugLogger.LogDebugMessage("OrderQuery request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口提交数据
            DebugLogger.LogDebugMessage("OrderQuery response : " + response);
        ...
        */
        public static WxPayData Refund(WxPayData inputObj, int timeOut = 6)
        {
            ...
            DebugLogger.LogDebugMessage("WxPayApi" + "Refund request : " + xml);
            string response = HttpService.Post(xml, url, true, timeOut);//调用HTTP通信接口提交数据到API
            DebugLogger.LogDebugMessage("WxPayApi" + "Refund response : " + response);
        ...
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6)
        {
            ...
            //签名
            inputObj.SetValue("sign", inputObj.MakeSign("MD5").ToUpper());
            string xml = inputObj.ToXml();

            //var start = DateTime.Now;

            DebugLogger.LogDebugMessage("WxPayApi UnfiedOrder request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);
            DebugLogger.LogDebugMessage("WxPayApi UnfiedOrder response : " + response);
        ...
        */
        public static string GenerateNonceStr()
        {
            ...
            return "ABC" + randomGenerator.GetRandomUInt().ToString() + ran.Next(999999) + ran.Next(999999);
        }
    }
}