...
namespace Sigbit.Data
{
    ...
    /// MySQL的数据库访问封装
    /// </summary>
    public class DBConnMySql5 : DBConnBase
    {
        ...
        private string SQLEncoding(string sSQL)
        {
            string sRet = Encoding.GetEncoding("latin1").GetString(Encoding.Default.GetBytes(sSQL));
        ...
        override public DataSet ExecuteDataSet(string sSQL)
        {
            ...
            sLimitSQL = SQLEncoding(sLimitSQL);
            ...
            lock (this)
            {
                ...
                adapter = new MySqlDataAdapter(sLimitSQL, _sqlConn);
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        dr[i] = Encoding.Default.GetString(Encoding.GetEncoding("latin1").GetBytes(dr[i].ToString()));
                    }
                }
