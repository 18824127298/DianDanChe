...
var todayID="";
function preYear(){
	sOldClickDay=document.all(sOldClickDate).innerText;
	if(sOldClickDay=="") sOldClickDay="1";
	...
	for (var intDay = 0;intDay < tableCal.rows[intWeek].cells.length;intDay++)
	{
		...
		if(day==daily)
		{
			...
			{ 
				...
				if(sOldClickDay==daily)  cell.className = "clickday";
				daily++;
			}
			else
				cell.innerText = "";
...
function setClickDate()
{
	...
	today = new getToday();
	if ((today.year == document.all.year.value) &&(today.month ==document.all.month.value))
	{
		if(sOldClickDate!="")
	...
	sOldClickDay=event.srcElement.innerText;
	...
	<body ONLOAD="newCalendar()" style="MARGIN:0px;OVERFLOW:auto">
		<TABLE cellspacing="0" cellpadding="0" width="100%" border="0" align="center">
			<TR height="30" bgcolor="#fdbf6e">
				<TD ALIGN="center" style="height: 30px"><INPUT id="btnPreYear" title="上一年" onclick="preYear();" type="button" value="<<" class="GoButton"><SELECT ID="year" ONCHANGE="newCalendar()"><SCRIPT LANGUAGE="JavaScript">
...
document.write("<OPTION VALUE= " + (intLoop + 1) + " " +(today.month == intLoop ?"Selected" : "") + ">" +months[intLoop]);
						...
						class="GoButton">
						<Input type="button" id="btnOK" value="确定" OnClick="getDate();" class="ShortButton"><Input type="button" id="btnEmpty" value="清空" OnClick="window.returnValue='';window.close();" class="ShortButton" title="返回空白"><Input type="button" value="取消" OnClick="Cancel();" class="ShortButton">
				</TD>
			</TR>
			<tr bgcolor="#8a622e">
				<td height="1">
...
document.write("</TR>");
		...
		<table id="tblTime" cellspacing="0" cellpadding="2" width="100%" border="0" bgcolor="white" align="center">
			<tr>
				<td nowrap width="15px"></td>
				<td nowrap><input id="chkReturnTime" type="checkbox" onclick="chkReturnTime_Onclick()">时间：</td>
				<td><input type="text" class="Text" disabled=true id="txtHour" value="00" style="WIDTH:22px" maxLength="2" onblur="CheckInputHour();"></td>
				<td nowrap>
				...
				<td><input type="text" class="Text" disabled=true id="txtMinute" value="00" style="WIDTH:22px" maxLength="2" onblur="CheckInputMinute();"></td>
				<td nowrap>
				...
				<td><input type="text" class="Text" disabled=true id="txtSecond" value="00" style="WIDTH:22px" maxLength="2" onblur="CheckInputSecond();"></td>
				<td nowrap>
