...
{
    /// <summary>
    /// 交易码 (dns_sys_trans_code)的表操作用户类
    /// </summary>
    public class TbSysTransCode : TbSysTransCodeBase
    {
        #region 用户可编辑区域
    ...
    /// <summary>
    /// 交易码 (dns_sys_trans_code)的字段名类
    /// </summary>
    public class TbSysTransCodeF
    {
        public const string TableName = "dns_sys_trans_code";
        public const string TransCode            = "trans_code";
        public const string TransCodeName        = "trans_code_name";
        public const string TransCodeDesc        = "trans_code_desc";
    ...
    /// <summary>
    /// 交易码 (dns_sys_trans_code)的表操作基类
    /// </summary>
    public class TbSysTransCodeBase : Sigbit.Data.TableBase
    {
        #region 私有变量定义
        protected string     _transCode               = "";
        protected string     _transCodeName           = "";
        protected string     _transCodeDesc           = "";
        ...
        public string TransCodeName
        {
            get
            {
                return _transCodeName;
            }
            set
            {
                _transCodeName = value;
            }
        }
        ...
        /// 交易码描述
        /// </summary>
        public string TransCodeDesc
        {
            get
            {
                return _transCodeDesc;
            }
            set
            {
                _transCodeDesc = value;
            }
        }
        ...
        public override string ToString()
        {
            ...
            sb.Append("TransCodeName: " + this._transCodeName + "<br>");
            sb.Append("TransCodeDesc: " + this._transCodeDesc + "<br>");
        ...
        public override bool Fetch(bool allowNoData)
        {
            ...
            //======= 1. 得到SQL语句 ==============
            sSQL  = "select trans_code_name,     trans_code_desc,     service_id,           \n";
            sSQL += "       create_time,         creator,             modify_time,          \n";
            sSQL += "       remarks              \n";
            sSQL += "  from dns_sys_trans_code    \n";
            sSQL += "  where trans_code = " + Quote(_transCode) + "\n";
            ...
            {
                if (!allowNoData)
                    throw new Exception("TbSysTransCode.Fetch() Error - cannot locate record via PrimaryKey.");
                else
                    return false;
            }
            ...
            _transCodeName  = DbToString(row["trans_code_name"]);
            _transCodeDesc  = DbToString(row["trans_code_desc"]);
        ...
        public override void Insert()
        {
            ...
            sSQL  = "insert into dns_sys_trans_code \n";
            sSQL += "( trans_code,       trans_code_name,  \n";
            sSQL += "  trans_code_desc,  service_id,       \n";
            sSQL += "  create_time,      creator,          \n";
            sSQL += "  modify_time,      remarks           \n";
            sSQL += ") values (          \n";
            sSQL += Quote(_transCode)       +","+ Quote(_transCodeName)   +",\n";
            sSQL += Quote(_transCodeDesc)   +","+ Quote(_serviceId)       +",\n";
        ...
        public override void Delete()
        {
            ...
            sSQL  = "delete from dns_sys_trans_code \n";
            sSQL += "  where trans_code = " + Quote(_transCode) + "\n";

            int nRowsAffected;
            nRowsAffected = DataHelper.Instance.ExecuteNonQuery(sSQL);
            if (nRowsAffected != 1)
                throw new Exception("TbSysTransCode.Delete() Error - cannot update data via PrimaryKey.");
        }
        ...
        public override void Update()
        {
            ...
            sSQL  = "update dns_sys_trans_code set \n";
            sSQL += " trans_code = " + Quote(_transCode) + ",\n";
            sSQL += " trans_code_name = " + Quote(_transCodeName) + ",\n";
            sSQL += " trans_code_desc = " + Quote(_transCodeDesc) + ",\n";
            ...
            nRowsAffected = DataHelper.Instance.ExecuteNonQuery(sSQL);
            if (nRowsAffected != 1)
                throw new Exception("TbSysTransCode.Update() Error - cannot update data via PrimaryKey.");
        }
        ...
        public override bool RecordExists()
        {
            ...
            //======= 1. 得到SQL语句 ==============
            sSQL = "select count(*) from dns_sys_trans_code \n";
            sSQL += "  where trans_code = " + Quote(_transCode) + "\n";
        ...
        /// </summary>
        public void FetchByE(string transCode)
        {
            bool hasData;
            hasData = FetchBy(transCode);
            if (!hasData)
                throw new Exception("TbSysTransCode.FetchBy(...) Error - cannot locate record via PrimaryKey.");
        }
        ...
        /// </summary>
        /// <returns>是否访问到数据</returns>
        public bool FetchBy(string transCode)
        {
        ...
        static public TbSysTransCode CreateBy(string transCode)
        {
            TbSysTransCode tbl;
            bool hasData;

            tbl = new TbSysTransCode();
            hasData = tbl.FetchBy(transCode);
        ...
        /// 以主键为参数删除一条数据
        /// </summary>
        static public void DeleteBy(string transCode)
        {
            TbSysTransCode tbl;
            tbl = new TbSysTransCode();

            tbl.TransCode = transCode;
        ...
        public override void DumpToFile(string fileName)
        {
            ...
            sLine = "trans_code_name\x9" + _transCodeName;
            writer.WriteLine(sLine);

            sLine = "trans_code_desc\x9" + _transCodeDesc;
            writer.WriteLine(sLine);
        ...
        public override void DumpAllRecordsToFile(string fileName)
        {
            ...
            //======== 2. 写入第一行（标题行） ========
            sLine  = "trans_code\ttrans_code_name\ttrans_code_desc\t";
            sLine += "service_id\tcreate_time\tcreator\t";
            sLine += "modify_time\tremarks";
            writer.WriteLine(sLine);

            //======== 3. 得到SQL语句 ========
            sSQL  = "select trans_code,      trans_code_name, trans_code_desc, \n";
            sSQL += "       service_id,      create_time,     creator,         \n";
            sSQL += "       modify_time,     remarks         \n";
            sSQL += "  from dns_sys_trans_code";
        ...
        static public string DBTypeOfFieldName(string fieldName)
        {
            switch (fieldName)
            {
                case "trans_code":
                    return "varchar";
                case "trans_code_name":
                    return "varchar";
                case "trans_code_desc":
                    return "text";
                case "service_id":
                    return "varchar";
                case "create_time":
                ...
                case "remarks":
                    return "varchar";
                default:
                    throw new Exception("TbSysTransCodeBase.DBTypeOfFieldName Error : meet unexpected fieldName - " + fieldName);
            }
        }
        ...
        static public string CSharpTypeOfFieldName(string fieldName)
        {
            switch (fieldName)
            {
                ...
                case "remarks":
                    return "string";
                default:
                    throw new Exception("TbSysTransCodeBase.CSharpTypeOfFieldName Error : meet unexpected fieldName - " + fieldName);
            }
        }
        ...
        static public string GetFieldValueBy(string fromFieldName, string fromFieldValue, string toFieldName)
        {
            ...
            //======= 1. 得到SQL语句 ==============
            sSQL  = "select " + toFieldName + " from dns_sys_trans_code \n";
