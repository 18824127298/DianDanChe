...
namespace Sigbit.App.Net.IBXService.VCodeBreak.Service
{
    public class IBXVCodeBreakFetchResultService
    {
        ...
        public IBMVCodeBreakResultFetchRESP DealWithRequest(IBMVCodeBreakResultFetchREQ reqMess, byte[] bsPacket)
        {
            ...
            //========== 1. 消息的处理 ==============
            TbLogVcodeBreak tblLogBreak = FetchOneISRResultRecordAndUpdate();

            if (tblLogBreak != null)
            {
                fetchRESP.UploadReceipt = tblLogBreak.LogUid;
                fetchRESP.RequestThirdId = tblLogBreak.RequestThirdId;
        ...
        private TbLogVcodeBreak FetchOneISRResultRecordAndUpdate()
        {
            string sSQL = "select * from vcb_log_vcode_break where current_status = 'broken' limit 1";
